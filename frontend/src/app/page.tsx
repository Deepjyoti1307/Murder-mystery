...
export default function LandingPage() {
  return (
    <div className="bg-background text-on-surface font-body-md min-h-screen flex flex-col relative overflow-x-hidden selection:bg-crimson-glare selection:text-white texture-overlay">
      ...
      {/* TopAppBar */}
      <header className="absolute top-0 left-0 w-full z-50 flex justify-between items-center px-4 md:px-8 lg:px-16 h-16 md:h-20 bg-transparent">
        <div className="flex items-center gap-2 md:gap-4">
          <Image src={Logo} alt="TechTrix Logo" width={32} height={32} className="object-contain md:w-10 md:h-10" />
          <div className="flex flex-col md:flex-row items-start md:items-center gap-1 md:gap-4">
            <span className="font-headline-xl text-base md:text-xl lg:text-2xl text-on-surface tracking-widest uppercase drop-shadow-[0_0_10px_rgba(220,20,60,0.5)]">
              TechTrix 2026
            </span>
          </div>
        </div>
        <div className="flex items-center gap-3 md:gap-6">
          <Link href="/admin/login" className="flex items-center gap-1.5 text-white bg-zinc-900 hover:bg-blood-red px-4 md:px-8 py-2 md:py-2.5 border border-blood-red/40 text-[10px] md:text-xs font-bold uppercase tracking-wider md:tracking-[0.2em] transition-all shadow-[0_0_15px_rgba(220,20,60,0.1)] rounded-full">
          ...
          <SignedIn>
            <UserButton appearance={{ elements: { userButtonAvatarBox: "w-8 h-8 md:w-10 md:h-10 border-2 border-blood-red" } }} />
          </SignedIn>
        </div>
      </header>
      ...
      <main className="flex-1 flex flex-col relative w-full">
        ...
        <section className="relative min-h-[100vh] flex items-center justify-center overflow-hidden bg-void-black pt-24 pb-32">
          ...
          {/* Content Container */}
          <div className="relative z-20 container mx-auto px-4 md:px-8 lg:px-16 text-center max-w-container-max-width">

            <div className="max-w-4xl mx-auto flex flex-col items-center">
              <h1 className="font-display-lg text-4xl sm:text-5xl md:text-6xl lg:text-8xl text-on-surface uppercase mb-6 md:mb-8 drop-shadow-[0_0_10px_rgba(220,20,60,0.8)] glitch-effect tracking-wider md:tracking-widest" data-text="MURDER MYSTERY">
                MURDER MYSTERY
              </h1>
              <div className="mb-8 md:mb-12 max-w-3xl text-center bg-black/60 p-4 md:p-6 border-l-4 border-blood-red shadow-[0_0_15px_rgba(220,20,60,0.5)]">
                <BreathingText
                  staggerDuration={0.08}
                  className="font-body-lg text-lg sm:text-xl md:text-2xl lg:text-3xl text-crimson-glare font-bold"
                >
                  The truth is buried in blood.
                </BreathingText>
              </div>
              <SignedOut>
                <Link href="/sign-in" className="bg-blood-red text-white font-headline-xl text-base sm:text-lg md:text-xl lg:text-2xl px-8 md:px-12 py-4 md:py-5 rounded-full uppercase tracking-wider md:tracking-widest shadow-[0_0_20px_rgba(220,20,60,0.8)] hover:shadow-[0_0_40px_rgba(220,20,60,1)] transition-all duration-500 border-2 border-crimson-glare group relative overflow-hidden flex items-center justify-center text-center animate-pulse hover:animate-none">
              ...
              <SignedIn>
                <Link href="/dashboard" className="bg-crimson-glare text-white font-headline-xl text-base sm:text-lg md:text-xl lg:text-2xl px-8 md:px-12 py-4 md:py-5 rounded-full uppercase tracking-wider md:tracking-widest shadow-[0_0_20px_rgba(220,20,60,0.8)] border-2 border-white/20 transition-all duration-500 hover:scale-105">
        ...
        {/* Footer */}
        <footer className="relative z-50 w-full bg-black/80 backdrop-blur-sm border-t border-blood-red/20 py-4 md:py-6 px-4 flex flex-wrap justify-center items-center gap-x-3 gap-y-2 mb-16 lg:mb-0">
          <div className="text-tertiary font-body-md text-[10px] md:text-sm text-center tracking-wider">
